...
from .mae.modeling.common import LayerNorm2d, MLPBlock
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self,
        ...
        print("embded dim:",embed_dim)
        print("decoder depth:",decoder_depth)
    ...
    def forward_encoder(self, x, mask, mask_ratio):
        ...
        x = self.sam_patch_embed(x)

        if self.sam_pos_embed is not None:
            x = x + self.sam_pos_embed
    ...
    def forward(self, imgs,imgs_1024, mask,mask_1024, mask_ratio=0.75):
        ...
        print("mask.shape:",mask.shape)
        print("mask_256.shape:",mask_256.shape)

        return imgs,pred_res,mask
    ...
    def forward_encoder_with_mask1(self, x, mask):
        ...
        mask1 = mask1.squeeze(1).squeeze(1)
        mask_small = mask_small.reshape(N, L).unsqueeze(1).unsqueeze(1)
        print("x.shape:",x.shape)
        print("mask.shape:",mask.shape)
        print("mask_small.shape:",mask_small.shape)
        return x, mask1, mask_small,encoder_feature,one_encoder_feature

    def forward_decoder_with_mask1(self, x, mask):
    ...
    def forward_test(self, imgs, mask):
        latent, new_mask, _,encoder_feature,one_encoder_feature = self.forward_encoder_with_mask1(imgs, mask)
        print("latent.shape:",latent.shape)
        print("new_mask.shape:",new_mask.shape)
        print("len(encoder_feature):",len(encoder_feature))
        image,decoder_feature = self.forward_decoder_with_mask1(latent, new_mask)
        print("image.shape:",image.shape)
        pred_img = self.unpatchify(image)
        return pred_img,new_mask,encoder_feature,one_encoder_feature,decoder_feature
...
def window_partition(x: torch.Tensor, window_size: int,ceshi_mask=False) -> Tuple[torch.Tensor, Tuple[int, int]]:
    ...
    if ceshi_mask==True and (pad_h > 0 or pad_w > 0):
        x = F.pad(x, (0, 0, 0, pad_w, 0, pad_h), value=1)
    ...
    def forward(self, x: torch.Tensor,mask=None) -> torch.Tensor:
        ...
        if self.window_size > 0:
            ...
            if mask is not None:
                ...
                print("mask partition shape:",mask.shape)
                print("---%%")
                print("mask.shape:", mask.shape)
...
class SAMAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor,mask=None) -> torch.Tensor:
        ...
        if mask is not None:
            ...
            mask = mask.repeat(1, self.num_heads, 1)
            mask = mask.view(-1, 1, H * W)
            print("attention after mask.shape:",mask.shape)
